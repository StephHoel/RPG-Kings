...
2. Validação de modelo de dados
   - Verificar que cenas testadas incluem `scheduled_day`, `weekdays` e `scheduled_hour` quando aplicável.

3. US1 — Agendamento pontual (scheduled_day)
   - Passos:
     1. Criar uma cena com `scheduled_day = 2` e `scheduled_hour = 14`.
...
4. US1 — Agendamento recorrente (weekdays)
   - Passos:
     1. Criar uma cena com `weekdays = ['Mon','Wed']` e `scheduled_hour = 18`.
...
5. US2 — Decisões e consequências
   - Passos:
     ...
     2. Verificar que o `PlayerState` é atualizado conforme o `Outcome` (resource_delta, flag_set ou branch).

6. Persistência
   - Passos:
     1. Realizar escolhas e salvar o jogo.
     2. Recarregar o save e verificar que decisões e estado persistiram.

7. Conflito / prioridade
   - Passos:
     1. Criar duas cenas programadas para o mesmo `day/hour` com diferentes `priority`.
