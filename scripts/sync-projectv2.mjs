...
async function rest(path, opts = {}) {
  const res = await fetch(`${GH_REST}${path}`, Object.assign({ headers: { Authorization: `bearer ${TOKEN}`, 'User-Agent': 'sync-projectv2-script' } }, opts))
  return res.json()
}
...
async function postComment(owner, repo, issueNumber, body) {
  try {
    ...
    const res = await fetch(`${GH_REST}${path}`, {
      method: 'POST',
      headers: { Authorization: `bearer ${TOKEN}`, 'User-Agent': 'sync-projectv2-script', 'Content-Type': 'application/json' },
...
async function withRetry(fn, attempts = 3, baseMs = 300) {
  ...
  for (let i = 0; i < attempts; i++) {
    try { return await fn() } catch (err) {
      ...
      console.error(`Attempt ${i + 1} failed, retrying after ${wait}ms:`, err && err.message ? err.message : err)
...
async function findIssueNodeId(owner, repo, issueNumber) {
  ...
  if (res && res.data && res.data.repository && res.data.repository.issue && res.data.repository.issue.id) return res.data.repository.issue.id
  // REST fallback
...
async function getProjectV2(owner, repo, number) {
  ...
  if (p.fields && p.fields.pageInfo && p.fields.pageInfo.hasNextPage) {
    ...
    while (cursor) {
      const res = await graphql(projectQ, { owner, repo, number: parseInt(number, 10), fieldsCursor: cursor })
      ...
      if (proj.fields.pageInfo && proj.fields.pageInfo.hasNextPage) cursor = proj.fields.pageInfo.endCursor
      else cursor = null
    }
  ...
  if (p.items && p.items.pageInfo && p.items.pageInfo.hasNextPage) {
    ...
    while (cursor) {
      const res = await graphql(projectQ, { owner, repo, number: parseInt(number, 10), itemsCursor: cursor })
      const repoObj = res && res.data && res.data.repository
      const proj = repoObj && repoObj.projectV2
      if (!proj || !proj.items) break
      allItems = allItems.concat(proj.items.nodes || [])
      if (proj.items.pageInfo && proj.items.pageInfo.hasNextPage) cursor = proj.items.pageInfo.endCursor
      else cursor = null
    }
...
async function updateItemStatus(itemId, fieldId, optionId) {
  ...
  return await withRetry(async () => await graphql(mutation, { input }))
}
...
async function main() {
  ...
  if (!owner || !repo || !issueNumber || !targetStatus) { usage(); process.exit(2) }
  ...
  if (PROJECT_ID && STATUS_FIELD_ID && optionId) {
    ...
    if (!itemId) { console.error('Could not obtain project item id using PROJECT_ID and issue node id', JSON.stringify(created, null, 2)); process.exit(7) }
    ...
    try {
      ...
      if (process.env.COMMENT_ON_RESULT === 'true') { await postComment(owner, repo, issueNumber, `Project board updated to **${targetStatus}** (item ${itemId}).`) }
    } catch (err) {
      console.error('Failed updating item status (env path):', err && err.message ? err.message : err)
      if (process.env.COMMENT_ON_RESULT === 'true') { await postComment(owner, repo, issueNumber, `Failed to update Project board to **${targetStatus}**: ${err && err.message ? err.message : String(err)}`) }
      process.exit(10)
    }
    return
  }
  ...
  if (!optionId) {
    const opt = (statusField.options && statusField.options.nodes) ? statusField.options.nodes.find(o => normalizeKey(o.name) === normalizedTarget) : null
