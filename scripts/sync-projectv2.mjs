...
async function main() {
  ...
  if (!project) { console.error('Project not found'); process.exit(3); }

  // Find status field
  let statusField = null;
  for (const f of project.fields.nodes) {
    if (f.__typename === 'ProjectV2SingleSelectField' && f.name && f.name.toLowerCase().includes('status')) {
      statusField = f; break;
    }
  }
  if (!statusField) { console.error('Status field not found'); process.exit(4); }

  // find option id
  const option = statusField.options.nodes.find(o => o.name.toLowerCase() === targetStatus.toLowerCase() || o.name.replace(/\s+/g,'').toLowerCase() === targetStatus.replace(/\s+/g,'').toLowerCase());
  if (!option) { console.error('Option not found for status:', targetStatus); console.error('Available:', statusField.options.nodes.map(o=>o.name)); process.exit(5); }

  // find item id for issue
  const item = project.items.nodes.find(it => it.content && it.content.number === issueNumber);
  if (!item) { console.error('Project item not found for issue', issueNumber); process.exit(6); }
